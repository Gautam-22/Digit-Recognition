// 234101014_DigitRecognition.cpp : Defines the entry point for the console application.
//
...
void load_codebook(){
	...
	fp=fopen("234101014_codebook.csv","r");
	if(fp==NULL){
		printf("Error in Opening File!\n");
		return;
	}
...
void generate_universe(){
	FILE* universefp;
	universefp=fopen("234101014_universe.csv","w");
	for(int d=0;d<=9;d++){
		for(int u=1;u<=TRAIN_SIZE;u++){
			char filename[40];
			_snprintf(filename,40,"234101014_dataset/234101014_E_%d_%d.txt",d,u);
...
void generate_codebook(){
	load_universe("234101014_universe.csv");
	LBG();
	store_codebook("234101014_codebook.csv",K);
}
...
void train_model(int digit, int utterance){
	...
	_snprintf(filenameA,40,"234101014_lambda/A_%d_%d.txt",digit,utterance);
	...
	_snprintf(filenameB,40,"234101014_lambda/B_%d_%d.txt",digit,utterance);
...
void calculate_avg_model_param(int d){
	...
	for(int u=1;u<=25;u++){
		...
		_snprintf(filenameA,40,"234101014_lambda/A_%d_%d.txt",d,u);
		fp=fopen(filenameA,"r");
		for (int i = 1; i <= N; i++)
		{
			for (int j = 1; j <= N; j++)
		...
		_snprintf(filenameB,40,"234101014_lambda/B_%d_%d.txt",d,u);
		fp=fopen(filenameB,"r");
		for (int i = 1; i <= N; i++)
		{
			for (int j = 1; j <= M; j++)
...
void store_final_lambda(int digit){
	...
	_snprintf(filenameA,40,"234101014_lambda/A_%d.txt",digit);
	fp=fopen(filenameA,"w");
	for (int i = 1; i <= N; i++)
	{
		for (int j = 1; j <= N; j++)
	...
	_snprintf(filenameB,40,"234101014_lambda/B_%d.txt",digit);
	fp=fopen(filenameB,"w");
	for (int i = 1; i <= N; i++)
	{
		for (int j = 1; j <= M; j++)
...
void processTestFile(int d){
	...
	_snprintf(filenameA,40,"234101014_lambda/A_%d.txt",d);
	fp=fopen(filenameA,"r");
	if (fp == NULL){
		printf("Error\n");
	}
	...
	_snprintf(filenameB,40,"234101014_lambda/B_%d.txt",d);
	fp=fopen(filenameB,"r");
	for (int i = 1; i <= N; i++){
		for (int j = 1; j <= M; j++){
			fscanf(fp, "%Lf ", &B[i][j]);
